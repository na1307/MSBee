    ...
    private string? OriginalPathToTool() {
        ...
        using (dotNetFramework) {
            if (dotNetFramework.GetValue("sdkInstallRootv1.1") is string sdkInstallRootv11) {
                resgenPath = Path.Combine(Path.Combine(sdkInstallRootv11, "Bin"), ToolName);
            }
        }
        ...
        if (string.IsNullOrEmpty(resgenPath)) {
            Log.LogError(
                ...
                ToolName, ToolLocationHelper.GetDotNetFrameworkVersionFolderPrefix(TargetDotNetFrameworkVersion.Version11),
